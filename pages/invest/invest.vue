<template>
	<view class="text-white">
		<u-navbar :title="i18n.invest" showback="true">


      <navigator url="/pages/invest/mine" slot="right" style="color: #2A64FB;">
		...
		<view class="m-30">
			...
			<view class="mt-30">
				<view class="box-shadow border-radius-20 mb-20 bg-black" v-for="item in invest">
					<text class="px-20 py-20 d-block u-border-bottom font-size-32" style="color: #000;">{{item.name}}</text>
					<view class="p-20">
						<view class="py-10">
							<view class="d-flex align-items-baseline">
								<text class="opacity-30 font-size-24" style="color: #000;">{{i18n.apr}}:</text>
								<text class="font-size-32 font-weight-bold ml-10" :style="{color:$downColor}">{{item.rate}}%</text>
							</view>
						</view>
						<view class="d-grid-columns-2  mt-8">
							<view class="d-flex align-items-baseline">
								<text class="opacity-30 font-size-24"  style="color: #000;">{{i18n.linkedReferencePrice}}:</text>
								<text class="ml-10" style="color: #000;">{{item.exercise_price}}</text>
							</view>
							<view class="d-flex align-items-baseline">
								<text class="opacity-30 font-size-24"  style="color: #000;">{{i18n.remainingShares}}:</text>
								<text class="ml-10" style="color: #000;">{{item.remaining_number + i18n.share}}</text>
							</view>
						</view>
						<view class="d-grid-columns-2 mt-8">
							<view class="d-flex align-items-baseline">
								<text class="opacity-30 font-size-24"  style="color: #000;">{{i18n.holdingDays}}:</text>
								<text class="ml-10" style="color: #000;">{{item.holdingDays + i18n.day}}</text>
							</view>
							<view class="d-flex align-items-baseline">
								<text class="opacity-30 font-size-24" style="color: #000;">{{i18n.maturityDate}}:</text>
								<text class="ml-10" :class="{'text-error' : item.buyStatus == 0}"  style="color: #000;">{{item.end_time}}</text>
							</view>
						</view>
						<view class="d-flex-between-center mt-20">
							<u-line-progress active-color="red" :percent="item.progress"></u-line-progress>
						</view>
						<button class="warning-button mt-20 py-0 font-size-28" v-if="item.buyStatus"
                    @click="$utils.jump('/pages/invest/purchase?id='+item.id)" style="color: #ffffff;">{{i18n.purchase}}</button>
					</view>
				</view>
