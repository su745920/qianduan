<template>
	<view class="text-white">
		<u-navbar :title="i18n.withdraw2" showback="true" :customBack="onBack">
		...
		<!--                  bg-color="#242424"></u-subsection>-->

		<view class="mx-36 py-40">
			<view class="bg-black px-0 py-16  border-radius-10">
				<view class="select-coin" @click="showSelect=true">
					...
					<view class="">
						<text class="iconfont icon-xiala text-ddd font-size-22 ml-16" style="color: #939393;"></text>
					</view>
				</view>
			</view>

			<u-action-sheet :list="coins" v-model="showSelect" v-if="currentNav===0" @click="confirmSelect"
				:cancelText="i18ncommon.cancel"></u-action-sheet>

			<u-action-sheet :list="coins1" v-model="showSelect" v-else @click="confirmSelect"
				:cancelText="i18ncommon.cancel"></u-action-sheet>


			<view class="d-flex font-size-34 mt-28 withdraw_type" v-if="false">
				<text class="mr-20 item" :class="type == 0 ? 'active' : ''"
					@click="type=0">{{i18n.withdrawToAddress}}</text>
				<!--
			...
			<view class="u-border-bottom  pb-16 withdraw-input-wrap pb-16 pt-28 " v-if="type == 0">
				<view class="d-flex-between-center">
					<text class="d-block font-size-32" style="color: #666;">{{i18n.withdrawAddress}}</text>
				</view>
				<!-- <view class="input-group" @click="showSelectAddress=true">
					<input type="text" v-if="selectAddress" :value="selectAddress.address"
						:placeholder="i18n.plsSelectWithdrawAddress" style="color: #ffffff;" disabled class="input">
					<text class="absolute-text iconfont icon-xiala text-white"></text>
				</view> -->
				<view class="input-group">
					<textarea type="text" v-model="selectAddress.address"
						:placeholder="$t('input_target_profit_price')" style="width: 100%;color: #ffffff;height:80rpx;line-height: 42rpx;" class="input">
						</textarea>
				</view>
			</view>
			<!-- 体现到银行卡 -->
			<view class="u-border-bottom  pb-16 withdraw-input-wrap pb-16 pt-28 " v-if="type == 1">
				<view class="d-flex-between-center">
					<text class="d-block font-size-32" style="color: #939393;">{{$t('c2c.bankcard')}}</text>
				</view>
				<view class="input-group">
					<input type="text" v-if="card && card.bank_account" :value="card.bank_account" disabled
						class="input" style="color: #000;">

					<button v-else class="warning-button py-0 font-size-22 w-50 mx-auto" style="color: #FFFFFF;"
						@click="$utils.jump('/pages/setting/bank')">{{i18n.addCard}}</button>
				</view>
			</view>
			<!-- 体現到银行卡 國際 -->
			<template v-if="type == 2">
				<view class="u-border-bottom  pb-16 withdraw-input-wrap pb-16 pt-28 ">
					...
					<view class="input-group">
						...
						<button v-else class="warning-button py-0 font-size-22 w-50 mx-auto" style="color: #FFFFFF"
							@click="$utils.jump('/pages/setting/bank_international')">{{i18n.addCard}}</button>
					</view>
				</view>
			</template>
			<!-- 转账数量 -->
			<view class="u-border-bottom  pb-16 withdraw-input-wrap pb-16 pt-28 ">
				<view class="d-flex-between-center">
					<text class="d-block font-size-32" style="color: #666;">{{i18n.withdrawNumber}}</text>
					<text style="color: #939393;">{{i18n.canUse}} {{legal_balance}} {{selectCoin.text}}</text>
				</view>
				<view class="input-group">
					<input type="digit" v-model="number" class="input" :placeholder="i18n.plsIptWithdrawNumber"
						style="color: #ffffff;" placeholder-style="color:#939393;font-size:14px" @blur="checkNumber">
					<text class="absolute-text" style="color: #ffffff;"
						@click="number=legal_balance;">{{i18n.all}}</text>
				</view>
			</view>
			<!-- 手续费 -->
			<text class="d-block text-right" style="height: 70rpx;line-height: 70rpx;">
				<text style="color: #939393;">{{i18n.leastNumber}}</text> <text class="ml-26"
					style="color: #ffffff;opacity: 1;">{{min_number}} {{selectCoin.text}}</text>
			</text>
			<!-- 到账数量 -->
			<view class="u-border-bottom  pb-16 withdraw-input-wrap pb-16 pt-28 ">
				<view class="d-flex-between-center">
			...
			<!-- 手续费 -->
			<text class="d-block text-right" style="height: 70rpx;line-height: 70rpx;">
				<text style="color: #939393;">{{i18n.handlingFee}}</text> <text class="ml-26"
					style="color: #ffffff;">{{(rate*number/100).toFixed(4)}} {{selectCoin.text}}</text>
			</text>
			<!-- 邮箱验证码 -->
			<view class="u-border-bottom  pb-16 withdraw-input-wrap pb-16 pt-28 ">
				<view class="d-flex-between-center">
					<text class="d-block font-size-32"
						style="color: #939393;">{{$t('setting.emailVerificationCode')}}</text>
				</view>
				<view class="input-group d-flex align-items-center">
					<input type="text" class="input flex-1" v-model="emailCode" :placeholder="$t('common.emailcodePlaceholder')"
						style="color: #ffffff;">
					<button size="mini" @click="getCode">{{ hasSend ? seconds + 's' : $t('common.send')}}</button>
				</view>
			</view>
			<!-- 谷歌验证码 -->
			<view v-if="user.google_secret" class="u-border-bottom  pb-16 withdraw-input-wrap pb-16 pt-28 ">
				<view class="d-flex-between-center">
					<text class="d-block font-size-32"
						style="color: #939393;">{{$t('google_code')}}</text>
				</view>
				<view class="input-group d-flex align-items-center">
					<input type="text" class="input flex-1" v-model="googleCode" :placeholder="$t('google_code_placeholder')"
						style="color: #ffffff;"></view>
			</view>
			<button class="warning-button mt-70 connect_btn" style="color: #000000;"
				@click="withdraw">{{$t("common.confirm")}}</button>
			...
			<u-popup v-model="showSelectAddress" mode="bottom">
				<view class="p-30">
					<block v-if="walletAddress.length">
						<view class="item" v-for="item in walletAddress" :key="item.id"
							@click="selectAddressFunc(item)">
							<view class="d-flex align-items-center" style="margin-top: 10px;">
								<image :src="item.upload_pic" class="border-radius-20 "
									style="width: 170rpx;height: 170rpx;"></image>
								<view style="flex: 1;" class="mx-20">
									<text class="font-size-28 font-weight-bold d-block" style="color: #ffffff;"><span
											style="color: #939393">{{i18n.coinType}}:</span> {{item.coin}}</text>
									<text class="font-size-28 text-warning font-weight-bold d-block"
										style="word-wrap:break-word;color: #ffffff;">
										<span style="color: #939393">{{i18n.address}}:</span> {{item.address}}</text>
									<text class="font-size-28 font-weight-bold d-block" style="color: #ffffff;"><span
											style="color: #939393">{{i18n.date}}:</span> {{item.created_at}}</text>
								</view>
								<text class="iconfont icon-checkbox-ok text-warning" v-if="selectAddress.id == item.id"
									style="color: #1FA2FF"></text>
								<text v-else class="iconfont icon-checkbox"></text>
							</view>
						</view>
					</block>
					<block v-else>
						<button class="w-50 mx-auto warning-button py-0 border-radius-20 text-white"
							style="color: #FFFFFF;height: 35px;line-height: 35px;"
							@click="$utils.jump('/pages/setting/addWallet')">{{$t('add_withdrawal_address')}}</button>
					</block>
		...
		methods: {
			...
			getCode() {
				...
				this.$u.throttle(() => {
					...
					if (hasSend) return
					if (!this.$u.test.email(this.user.email) || !this.user.email) {
			...
			getWalletInfo() {
				...
				if (currency > 0) {
					...
					this.$https.initDataToken({
						...
						this.rate = Number(res.withdraw_fee)
			...
			withdraw() {
				...
				let data = {
					...
					currency: currency,
					number: number,
					rate: (rate*number/100).toFixed(4),
					type: type,
					lang: lang,
					code: emailCode,
					googleCode: googleCode,
					usdtType: selectCoin.text
				};
			...
			getCardInternational() {
				...
				this.$https.initDataToken({
					...
					this.card_international = message;
				})
			}
		},
		computed: {
			...
			canGetNumber() {
				const value = (this.number - this.rate*this.number/100).toFixed(6)
