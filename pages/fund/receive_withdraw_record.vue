<template>
	<view>
		<u-navbar :title="$t('common.log')" showback="true"></u-navbar>
		<view class="mx-36">
			<view class="bg-black text-white my-30 p-20 border-radius-20" v-for="item in data" :key="item.id">
				<view class="d-flex justify-content-between align-items-center">
					...
					<text class="font-size-32" :style="{'color':$utils.getColor(item.type)}" style="opacity: 1;">{{item.amount}}</text>
				</view>
				<view class="text-right font-size-22 my-4">
					{{item.name}}
				</view>
				<view class="d-flex justify-content-between">
					<text :style="{'color':$utils.getColor(item.type)}"  style="opacity: 1;">{{item.type == -1 ? $t("home.withdraw") : $t("home.recharge")}}</text>
					<text style="color: #000;opacity: 0.6">{{item.created_at}}</text>
				</view>
			</view>
		</view>
	</view>
...
<script>
	export default {
        ...
        this.$https.initDataToken({
          ...
          let arr2 = ret2.message.map(item => {
            item.type = -1
