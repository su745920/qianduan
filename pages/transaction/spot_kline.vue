  ...
  <!--          <u-icon name="star" size="42" :color="$baseColor" v-else @click="addOptional"></u-icon>-->
    ...
    <block>
      <view class="footer font-size-28"  style="background: #1b1b1b;display: none;">
        ...
        <view style="display: flex;flex-direction: column;" @click="$utils.jump(`/pages/transaction/contract?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=0`,'reLaunch')">
...
export default {
  ...
  methods: {
	  onBack() {
		  if(this.from === 'market') {
			  this.$utils.jump('/pages/index/index','switchTab')
		  }else {
			  this.$utils.jump('/pages/transaction/currency')
		  }
	  },
	  goHome() {
		uni.reLaunch({
