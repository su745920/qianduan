<template>
	<view>
		<view class="options">
			<view class="item">
				<scroll-view :scroll-x="true" style="width: 300px;">
					<text class="mx-10" v-for="(item,index) in intervals2" @click="changeInterval(index)"></text>
				</scroll-view>
			</view>
				...
				this.$store.state.webScoket?.onMessage(evt => {
					 ...
					 if(k.type === 'message'){
					  ...
					  if(k.symbol?.toUpperCase() === coin && k.type === 'daymarket' && k.self === 1) {
						  ...
						  let symbolStr  = currencyName + '/USDT'
