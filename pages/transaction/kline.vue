<template>
  <view style="background: #000000;">
    ...
    <view class="mx-30 pt-30">
      <view class="d-flex-between-center" v-if="false">
      ...
      <view class="d-flex-between-center mt-30 text-white ">
        <view class="d-flex align-items-center font-weight-bold opacity-75">

          <u-icon name="arrow-leftward" size="38" color="#ffffff" @click="back"></u-icon>

          <u-icon name="list-dot" size="42" class="position-relative" style="top: 2rpx;margin-left: 20px;"  @click="showSelectCoin=true"></u-icon>
          <text class="font-size-40 mx-10" style="color: #FFFFFF"  @click="showSelectCoin=true">{{currency_name + '/' + legal_name}}</text>
          <text v-if="symbolQuotation.change" class="tag"  @click="showSelectCoin=true"
    ...
    <view class="d-flex-between-center p-30" style="height: 152rpx;">
        ...
        <view class="">
          <text class="d-block font-size-40 font-weight-bold"
                :style="{color:$utils.getColor(symbolQuotation1.change)}">{{symbolQuotation1.close | setPrecision(symbolQuotation.precision_length)}}</text>
...
export default {
  ...
  methods: {
    ...
    back(){
      ...
      const url = '/pages/transaction/contract'
      uni.reLaunch({
        url
      })
