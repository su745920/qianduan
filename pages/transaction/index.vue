	...
	<view style="background: #000000;">
		...
		<block v-if="currentNav == 0">
			<view class="footer font-size-28" v-if="symbolQuotation.close">
				<view class="btn btn1"  @click="$utils.jump(`/pages/transaction/contract?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=0`,'reLaunch')">
					<text class="d-block">{{i18n.buy + '/' + i18n.long}}</text>
				</view>
				<view class="btn btn2"  @click="$utils.jump(`/pages/transaction/contract?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=1`,'reLaunch')">
		...
		<block v-if="currentNav == 2">
			<view class="footer font-size-28" v-if="symbolQuotation.close">
				<view class="btn btn1" @click="$utils.jump(`/pages/transaction/currency?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=0`,'reLaunch')">
					<text class="d-block">{{i18n.buy}}</text>
				</view>
				<view class="btn btn2" @click="$utils.jump(`/pages/transaction/currency?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=1`,'reLaunch')">
				...
				<view class="p-30">
					...
					<view class="border-bottom-white py-16">
						...
						<view class="d-flex-between-center mt-16" v-if="buyDirection == 1">
							<text v-for="item in timers" class="tag w-19 text-center font-size-30 d-block" :class="currentTimer == item ? 'tag-success py-12' : 'tag-plain-success py-10'" @click="currentTimer=item">{{item}}s</text>
						</view>
						<view class="d-flex-between-center mt-16" v-else-if="buyDirection == 2">
							<text v-for="item in timers" class="tag w-19 text-center font-size-30 d-block" :class="currentTimer == item ? 'tag-error py-12' : 'tag-plain-error py-10'" @click="currentTimer=item">{{item}}s</text>
						</view>
					</view>
					<view class="py-16">
          ...
          this.microInsurance = Number(user_lever).toFixed(4)
			...
			getQuotation() {
				...
				this.$u.api.index.getQuotation().then(res => {
					...
					has.close = Number(has.close)
					has.high = Number(has.high)
					has.low = Number(has.low)

					has.precision_length = this.$utils.getPrecisionLength(has.close)
