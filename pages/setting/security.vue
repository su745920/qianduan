<template>
	<view class="text-white">
		<u-navbar :title="$t('setting.aqsz')" showback="true"></u-navbar>
		<view class="list">
			<navigator url="/pages/setting/password" class="item">
				<text style="color: #ffffff;">{{i18n.editLoginPassword}}</text>
		...
		<view class="list">
			<view class="item" @click="showEditPhone=true"  v-if="false">
				<text style="color: #ffffff;">{{i18n.bindPhone}}</text>
				<view class="d-flex align-items-center font-size-28 opacity-50">
					<text class="text mr-20" style="color: #ffffff;">{{user.phone}}</text>
					<text class="iconfont icon-gengduo1 font-size-28"></text>
				</view>
			</view>
			<view class="item" v-if="false">
				<text style="color: #ffffff;"{{i18n.email}}</text>
				<text class="text  font-size-28 opacity-50" style="color: #ffffff;">{{user.email}}</text>
			</view>
		</view>
		<!-- <view class="list">
			<view class="item">
		...
		<u-popup v-model="showEditPhone"  mode="bottom" length="50%" :title="i18n.bindPhone">
			<view class="mx-30">
				...
				<view class="d-flex border-radius-20 bg-secondary py-20 px-30 position-relative">
					...
					<button class="warning-button py-0 font-size-22 px-20" v-if="!hasSend" @click="sendCode" style="color: #FFFFFF">{{i18n.sendCode}}</button>
					<button class="secondary-button py-0 font-size-22 px-20" v-else>{{seconds}}s</button>
				</view>
				<text class="d-block font-size-32 mt-30 mb-8" style="color: #000;">{{i18n.code}}</text>
				<view class="border-radius-20 bg-secondary py-20 px-30 position-relative ">

					<input type="digit" v-model="code" class="security" :placeholder="i18n.plsIptCode"  style="color: #000;">
				</view>
				<button class="warning-button mt-40 font-size-28" @click="saveMobile"  style="color: #ffffff;">{{$t("common.submit")}}</button>
			</view>
		</u-popup>
		<u-select v-model="showSelectArea" :list="areas" value-name="area_code"  style="color: #000;"
              :cancelText='$t("common.cancel")' :confirmText='$t("common.confirm")' @confirm="confirmSelectArea"></u-select>
