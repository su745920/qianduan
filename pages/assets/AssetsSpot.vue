    ...
    <view   style="background: #000000;border-top: 6px solid #121212;">
      <view class="m-30">
        <view  v-for="item in showList">
          <view class="p-30 mb-10 text-white"
            ...
            <view style="display: flex;flex-direction: column;">
              <view style="color: #ffffff;font-weight: 500;font-size: 15px;margin-top: 5px;"  v-if="item.currency_name==='USDT'"> {{Number(item.change_balance).toFixed(6)}} </view>
              <view style="color: #ffffff;font-weight: 500;font-size: 15px;margin-top: 5px;"  v-else> {{Number(item.usdt_balance).toFixed(6)}} </view>
    ...
    getSpotWalletList() {
      ...
      this.$https.initDataToken({
        ...
        this.originalList.change_wallet.balance.forEach(item => {
		  ...
		  item.usdt_balance = Number(item.change_balance)
