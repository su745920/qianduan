  ...
  methods: {
    ...
    getFuturesWalletList() {
      ...
      this.$https.initDataToken({
        ...
        const convert =  Number(res.lever_wallet
            .totle);
        let lever_walletUSDT = Number(res.lever_wallet
            .totle);
        ...
        this.originalList.lever_wallet.balance.forEach(item => {
          ...
          if (item.currency_name==='USDT' || item.currency_name==='USDC'){
            item.usdt_balance2 =Number(item.lever_balance).toFixed(6);
          }else {
            // item.usdt_balance2 = parseFloat((Number(item.lever_balance) * Number(item
            //     .usdt_price)).toFixed(4))
			item.usdt_balance2 = Number(item.lever_balance)
          }
        });
