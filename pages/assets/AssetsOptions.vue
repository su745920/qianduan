  ...
  <view style="background: #000000;">
    <view class="m-20" style="background: #000000;">
      <view
        ...
        <view class="d-block font-size-22 mt-8" style="color: #000;margin-left: 20px;display: flex;">
    ...
    <view style="background: #000000;border-top: 6px solid #121212;">
      <view class="mt-30">
		  <view  v-for="item in showList">
		    <view v-if="item.currency_name ==='USDT'" class="p-30 mb-10 text-white" style="display: flex;justify-content: space-between;">
            ...
            <image v-if="item.currency_name==='USDT' "
		      ...
		      <view style="display: flex;flex-direction: column;">
		        <view style="color: #ffffff;font-weight: 500;font-size: 15px;margin-top: 5px;"> {{Number(item.usdt_balance2).toFixed(6)}} </view>
  ...
  methods: {
    todayProfitLossFn(){
      ...
      that.$https.initDataToken({
        ...
        that.todayProfitLoss=Number(res.p).toFixed(6);
    ...
    getOptionsWalletList() {
      let lang=uni.getStorageSync("lang")||"en";
      this.$https.initDataToken({
        url: 'wallet/optionWalletList?lang='+lang,
        type: 'POST'
      }, (res, msg) => {
        this.originalList = res
        const convert = Number(res
            .micro_wallet.totle);

        let micro_walletUSDT = Number(res
            .micro_wallet.totle);
        ...
        this.originalList.micro_wallet.balance.forEach(item => {
          ...
          if (currency_name==="USDT"){
            item.usdt_balance2 = Number(item.micro_balance).toFixed(6);
          }else{
            // item.usdt_balance2 = parseFloat((Number(item.micro_balance) * Number(item
            //     .usdt_price)).toFixed(2))
			item.usdt_balance2 = Number(item.micro_balance)
          }
