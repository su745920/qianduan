      ...
      <view class="item overflow-hidden">
        <scroll-view :scroll-x="true" style="white-space: nowrap;">
          <text class="mx-10" v-for="(item,index) in intervals"
                ...
                @click="changeInterval(index)">{{ item.name }}
          </text>
        </scroll-view>
      </view>
      <!-- <view class="item overflow-hidden">
...
export default {
  ...
  methods: {
    ...
    getHistoryDataV2() {
      ...
      let url = "http://154.204.59.61/api/currency/new_timeshar_v5?period=" + period + "&size=200&symbol=" + symbol;
      let data = [];
      this.$u.api.market.getHistoryDataV2(200, symbol, period).then(response => {
        ...
        if (data && data.length > 0) {
          ...
          let chg = 100 * (lastData.close - lastData.open) / lastData.open;
          chg = chg.toFixed(2);
          lastData.change = chg;
          that.$emit('getSocketData', lastData);
    ...
    subMarket() {
      let that = this;
      var socket = new SockJS('http://154.204.59.61/market/market-ws');
      ...
      stompClient.connect({}, function (frame) {
        stompClient.subscribe('/topic/bibi/kline/btcusdt', function (msg) {
          ...
          const data = {
            id: Number(candlestick.time)*1000,
            timestamp: Number(candlestick.time)*1000,
...
.options {
  border-top: 2 rpx solid #333;
  border-bottom: 2 rpx solid #333;
  display: grid;
